...
def get_latest_two_archives() -> tuple:
    ...
    files = sorted([f for f in os.listdir(archive_dir) if f.startswith("japanese_datasets_") and f.endswith(".json")])
...
def generate_changelog() -> Dict:
    ...
    # 날짜 정보 추출
    previous_date = os.path.basename(previous_file).replace("japanese_datasets_", "").replace(".json", "")
    current_date = os.path.basename(current_file).replace("japanese_datasets_", "").replace(".json", "")
